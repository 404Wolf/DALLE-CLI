...
def configure_openai():
    ...
    if not path.exists(config_file):
        ...
        if not api_key:
            ...
            with open(config_file, "w", encoding="UTF-8") as f:
                f.write(f'{{"api_key": "{api_key}"}}')
    if not api_key and not isinstance(getenv("OPENAI_API_KEY"), str):
        raise NameError("OPENAI_API_KEY not found in enviornment.")
    client.api_key = str(api_key or getenv("OPENAI_API_KEY"))
        ...
        with console.status("Generating image...", spinner="dots8Bit"):
            for _ in range(iterations):
                ...
                if response.data[0].url is None:
                    raise TypeError("OpenAI response type was invalid.")
